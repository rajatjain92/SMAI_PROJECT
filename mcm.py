...
for iter_no in range(num_iter):
    ...
    val_data_r[:, rand_sign] = data[:, val_perm][:, rand_sign]
    val_label[rand_sign] *= -1


    s = svm.MySVM(kernel = lambda x, y: svm.rbf_kernel(x, y, 0.01), c = 10)
    s.train(tr_data, tr_label)
    prediction = np.sign(s.predict(val_data, True) - s.predict(val_data_r, True))
